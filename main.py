...
LAST_MATCHES: Dict[str, List[Dict[str, Any]]] = defaultdict(list) # Memoria de últimos matches por sesión (para pedidos de "esos productos / links")
...
# Pedidos explícitos de recomendación (imperativos y frases comunes, con variantes locales)
...
# Palabras de compra/acción y de solicitud de enlaces que también cuentan como intención
...
- Solo respondes dentro de ese contexto; si te preguntan algo fuera, redirige suavemente.
...
- Si el usuario pide "links", "acceso directo", "ver producto" o similar, explícale brevemente que al final verá las tarjetas con los enlaces correspondientes (no inventes enlaces en el texto).
...
async def chat_endpoint(payload: ChatPayload, request: Request):
    ...
    # 2) Matches y tarjetas
    matches: List[Dict[str, Any]] = []
    if recommend_now:
        # Intentamos con la consulta actual
        matches = match_products(user_msg, topk=3)

        # Si no encontró nada y anteriormente sí, usamos los últimos
        if not matches and LAST_MATCHES.get(session_id):
            matches = LAST_MATCHES[session_id]
    ...
    # 7) Actualizar memoria de diálogo
    history.append({"role": "user", "content": user_msg})
    history.append({"role": "assistant", "content": base_answer})

    # 7.1) Guardar últimos matches si hubo
    if recommend_now and matches:
        LAST_MATCHES[session_id] = matches
